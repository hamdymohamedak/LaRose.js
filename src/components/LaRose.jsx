...
export function SeeMore({
  ...
  useEffect(() => {
    if (textRef.current) {
      setContentHeight(isExpanded ? `${textRef.current.scrollHeight}px` : 'auto');
    }
  }, [isExpanded, text]);
...
.content.collapsed {
  max-height: 3rem; /* يمكن تعديل هذا بناءً على حجم المحتوى */
  opacity: 0.5;
}

.content.expanded {
  max-height: 50rem; /* يمكن تعديل هذا بناءً على حجم المحتوى */
  opacity: 1;
}
